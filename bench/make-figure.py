...
def make_title(title: str):
    return {"text": title, "x": 0.5, "y": 0.95, "xanchor": "center", "yanchor": "top"}
...
def add_cache_vline(fig, cache, name, color, len_min, len_max):
    ...
    fig.add_vline(cache, line_width=2, line_dash="dash", line_color=color)

    fig.add_annotation(
        x=(math.log(cache)) / math.log(10),
        y=2,
        showarrow=False,
        xshift=-15,
...
def plot_sort_types_frame(df, title, args, caches):
    ...
    fig.update_layout(title=make_title(title), yaxis_tickangle=-30)
...
def plot_vxsort_vs_all_frame(df, args):
    ...
    fig.update_layout(
        title=make_title(f"vxsort vs. others {title_suffix}"),
        bargap=0.3,
        bargroupgap=0.2,
        yaxis_tickangle=-30,
    )
