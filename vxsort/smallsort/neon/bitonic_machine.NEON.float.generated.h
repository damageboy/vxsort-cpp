/////////////////////////////////////////////////////////////////////////////
////
// This file was auto-generated by a tool at 2021-03-01 07:58:04
//
// It is recommended you DO NOT directly edit this file but instead edit
// the code-generator that generated this source file instead.
/////////////////////////////////////////////////////////////////////////////

#ifndef BITONIC_MACHINE_NEON_FLOAT_H
#define BITONIC_MACHINE_NEON_FLOAT_H

#include <cassert>
#include <limits>
#include <arm_neon.h>
#include "../bitonic_machine.h"

#define i2d _mm256_castsi256_pd
#define d2i _mm256_castpd_si256
#define i2s _mm256_castsi256_ps
#define s2i _mm256_castps_si256
#define s2d _mm256_castps_pd
#define d2s _mm256_castpd_ps

namespace vxsort {
namespace smallsort {

template<> struct bitonic_machine<float, AVX2> {
    static const int N = 8;
    static constexpr float MAX = std::numeric_limits<float>::max();
public:
    typedef float32x4_t TV;
    typedef float32x4_t TMASK;

    static INLINE void sort_01v_ascending(TV& d01) {
        TV min, max, s;

        s = i2s(vrev64q_s32(s2i(d01)));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b01100110));

        s = i2s(_mm256_shuffle_epi32(s2i(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b00111100));

        s = i2s(vrev64q_s32(s2i(d01)));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b01011010));

        s = d2s(_mm256_permute4x64_pd(s2d(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b11110000));

        s = i2s(_mm256_shuffle_epi32(s2i(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b11001100));

        s = i2s(vrev64q_s32(s2i(d01)));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b10101010));
    }
    static INLINE void merge_01v_ascending(TV& d01) {
        TV min, max, s;

        s = d2s(_mm256_permute4x64_pd(s2d(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b11110000));

        s = i2s(_mm256_shuffle_epi32(s2i(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b11001100));

        s = i2s(vrev64q_s32(s2i(d01)));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b10101010));
    }
    static INLINE void sort_01v_descending(TV& d01) {
        TV min, max, s;

        s = i2s(vrev64q_s32(s2i(d01)));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b10011001));

        s = i2s(_mm256_shuffle_epi32(s2i(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b11000011));

        s = i2s(vrev64q_s32(s2i(d01)));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b10100101));

        s = d2s(_mm256_permute4x64_pd(s2d(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b00001111));

        s = i2s(_mm256_shuffle_epi32(s2i(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b00110011));

        s = i2s(vrev64q_s32(s2i(d01)));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b01010101));
    }
    static INLINE void merge_01v_descending(TV& d01) {
        TV min, max, s;

        s = d2s(_mm256_permute4x64_pd(s2d(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b00001111));

        s = i2s(_mm256_shuffle_epi32(s2i(d01), 0b01'00'11'10));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b00110011));

        s = i2s(vrev64q_s32(s2i(d01)));
        min = _mm256_min_ps(s, d01);
        max = _mm256_max_ps(s, d01);
        d01 = i2s(_mm256_blend_epi32(s2i(min), s2i(max), 0b01010101));
    }
    static INLINE void sort_02v_ascending(TV& d01, TV& d02) {
        TV tmp;

        sort_01v_ascending(d01);
        sort_01v_descending(d02);

        tmp = d02;
        d02 = _mm256_max_ps(d01, d02);
        d01 = _mm256_min_ps(d01, tmp);

        merge_01v_ascending(d01);
        merge_01v_ascending(d02);
    }
    static INLINE void sort_02v_descending(TV& d01, TV& d02) {
        TV tmp;

        sort_01v_descending(d01);
        sort_01v_ascending(d02);

        tmp = d02;
        d02 = _mm256_max_ps(d01, d02);
        d01 = _mm256_min_ps(d01, tmp);

        merge_01v_descending(d01);
        merge_01v_descending(d02);
    }
    static INLINE void merge_02v_ascending(TV& d01, TV& d02) {
        TV tmp;

        tmp = d01;
        d01 = _mm256_min_ps(d02, d01);
        d02 = _mm256_max_ps(d02, tmp);

        merge_01v_ascending(d01);
        merge_01v_ascending(d02);
    }
    static INLINE void merge_02v_descending(TV& d01, TV& d02) {
        TV tmp;

        tmp = d01;
        d01 = _mm256_min_ps(d02, d01);
        d02 = _mm256_max_ps(d02, tmp);

        merge_01v_descending(d01);
        merge_01v_descending(d02);
    }
    static INLINE void sort_03v_ascending(TV& d01, TV& d02, TV& d03) {
        TV tmp;

        sort_02v_ascending(d01, d02);
        sort_01v_descending(d03);

        tmp = d03;
        d03 = _mm256_max_ps(d02, d03);
        d02 = _mm256_min_ps(d02, tmp);

        merge_02v_ascending(d01, d02);
        merge_01v_ascending(d03);
    }
    static INLINE void sort_03v_descending(TV& d01, TV& d02, TV& d03) {
        TV tmp;

        sort_02v_descending(d01, d02);
        sort_01v_ascending(d03);

        tmp = d03;
        d03 = _mm256_max_ps(d02, d03);
        d02 = _mm256_min_ps(d02, tmp);

        merge_02v_descending(d01, d02);
        merge_01v_descending(d03);
    }
    static INLINE void merge_03v_ascending(TV& d01, TV& d02, TV& d03) {
        TV tmp;

        tmp = d01;
        d01 = _mm256_min_ps(d03, d01);
        d03 = _mm256_max_ps(d03, tmp);

        merge_02v_ascending(d01, d02);
        merge_01v_ascending(d03);
    }
    static INLINE void merge_03v_descending(TV& d01, TV& d02, TV& d03) {
        TV tmp;

        tmp = d01;
        d01 = _mm256_min_ps(d03, d01);
        d03 = _mm256_max_ps(d03, tmp);

        merge_02v_descending(d01, d02);
        merge_01v_descending(d03);
    }
    static INLINE void sort_04v_ascending(TV& d01, TV& d02, TV& d03, TV& d04) {
        TV tmp;

        sort_02v_ascending(d01, d02);
        sort_02v_descending(d03, d04);

        tmp = d03;
        d03 = _mm256_max_ps(d02, d03);
        d02 = _mm256_min_ps(d02, tmp);

        tmp = d04;
        d04 = _mm256_max_ps(d01, d04);
        d01 = _mm256_min_ps(d01, tmp);

        merge_02v_ascending(d01, d02);
        merge_02v_ascending(d03, d04);
    }
    static INLINE void sort_04v_descending(TV& d01, TV& d02, TV& d03, TV& d04) {
        TV tmp;

        sort_02v_descending(d01, d02);
        sort_02v_ascending(d03, d04);

        tmp = d03;
        d03 = _mm256_max_ps(d02, d03);
        d02 = _mm256_min_ps(d02, tmp);

        tmp = d04;
        d04 = _mm256_max_ps(d01, d04);
        d01 = _mm256_min_ps(d01, tmp);

        merge_02v_descending(d01, d02);
        merge_02v_descending(d03, d04);
    }
    static INLINE void merge_04v_ascending(TV& d01, TV& d02, TV& d03, TV& d04) {
        TV tmp;

        tmp = d01;
        d01 = _mm256_min_ps(d03, d01);
        d03 = _mm256_max_ps(d03, tmp);

        tmp = d02;
        d02 = _mm256_min_ps(d04, d02);
        d04 = _mm256_max_ps(d04, tmp);

        merge_02v_ascending(d01, d02);
        merge_02v_ascending(d03, d04);
    }
    static INLINE void merge_04v_descending(TV& d01, TV& d02, TV& d03, TV& d04) {
        TV tmp;

        tmp = d01;
        d01 = _mm256_min_ps(d03, d01);
        d03 = _mm256_max_ps(d03, tmp);

        tmp = d02;
        d02 = _mm256_min_ps(d04, d02);
        d04 = _mm256_max_ps(d04, tmp);

        merge_02v_descending(d01, d02);
        merge_02v_descending(d03, d04);
    }
    static INLINE void cross_min_max(TV& d01, TV& d02) {
        TV tmp;

        tmp = d2s(_mm256_permute4x64_pd(i2d(_mm256_shuffle_epi32(s2i(d02), 0b00'01'10'11)), 0b01'00'11'10));
        d02 = _mm256_max_ps(d01, tmp);        
        d01 = _mm256_min_ps(d01, tmp);    
    }
    static INLINE void strided_min_max(TV& dl, TV& dr) {
        TV tmp;
        
        tmp = dl;
        dl = _mm256_min_ps(dr, dl);
        dr = _mm256_max_ps(dr, tmp);
    }

#ifdef BITONIC_TESTS

    // This is generated for testing purposes only
    static NOINLINE void sort_01v_full_ascending(float *ptr) {
        TV d01 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 0));
        sort_01v_ascending(d01);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 0), d01);
    }

    // This is generated for testing purposes only
    static NOINLINE void sort_02v_full_ascending(float *ptr) {
        TV d01 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 0));
        TV d02 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 1));
        sort_02v_ascending(d01, d02);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 0), d01);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 1), d02);
    }

    // This is generated for testing purposes only
    static NOINLINE void sort_03v_full_ascending(float *ptr) {
        TV d01 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 0));
        TV d02 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 1));
        TV d03 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 2));
        sort_03v_ascending(d01, d02, d03);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 0), d01);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 1), d02);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 2), d03);
    }

    // This is generated for testing purposes only
    static NOINLINE void sort_04v_full_ascending(float *ptr) {
        TV d01 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 0));
        TV d02 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 1));
        TV d03 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 2));
        TV d04 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 3));
        sort_04v_ascending(d01, d02, d03, d04);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 0), d01);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 1), d02);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 2), d03);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 3), d04);
    }

    // This is generated for testing purposes only
    static NOINLINE void sort_01v_full_descending(float *ptr) {
        TV d01 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 0));
        sort_01v_descending(d01);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 0), d01);
    }

    // This is generated for testing purposes only
    static NOINLINE void sort_02v_full_descending(float *ptr) {
        TV d01 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 0));
        TV d02 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 1));
        sort_02v_descending(d01, d02);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 0), d01);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 1), d02);
    }

    // This is generated for testing purposes only
    static NOINLINE void sort_03v_full_descending(float *ptr) {
        TV d01 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 0));
        TV d02 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 1));
        TV d03 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 2));
        sort_03v_descending(d01, d02, d03);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 0), d01);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 1), d02);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 2), d03);
    }

    // This is generated for testing purposes only
    static NOINLINE void sort_04v_full_descending(float *ptr) {
        TV d01 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 0));
        TV d02 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 1));
        TV d03 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 2));
        TV d04 = _mm256_loadu_ps((float const *) ((__m256 const *) ptr + 3));
        sort_04v_descending(d01, d02, d03, d04);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 0), d01);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 1), d02);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 2), d03);
        _mm256_storeu_ps((float *) ((__m256 *)  ptr + 3), d04);
    }

    // This is generated for testing purposes only
    static void sort_full_vectors_ascending(float *ptr, size_t length) {
        assert(length % N == 0);
        switch(length / N) {
            case 1: sort_01v_full_ascending(ptr); break;
            case 2: sort_02v_full_ascending(ptr); break;
            case 3: sort_03v_full_ascending(ptr); break;
            case 4: sort_04v_full_ascending(ptr); break;
        }
    }

    // This is generated for testing purposes only
    static void sort_full_vectors_descending(float *ptr, size_t length) {
        assert(length % N == 0);
        switch(length / N) {
            case 1: sort_01v_full_descending(ptr); break;
            case 2: sort_02v_full_descending(ptr); break;
            case 3: sort_03v_full_descending(ptr); break;
            case 4: sort_04v_full_descending(ptr); break;
        }
    }

#endif

};
}
}

#undef i2d
#undef d2i
#undef i2s
#undef s2i
#undef s2d
#undef d2s
