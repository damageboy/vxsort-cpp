...
null_shuffle_ps_imm8 = _MM_SHUFFLE(1, 0, 1, 0)  # pos0: op1[0], pos1: op1[1], pos2: op2[0], pos3: op2[1]
...
def test_mm256_permute_epi32_null_permute_works():
    s = Solver()
    input_vector = ymm_reg("ymm0")
    output_vector = _mm256_permute_ps(input_vector, null_permute_epi32_imm8)
...
def test_mm256_permute_epi32_null_permute_found():
    s = Solver()
...
def test_mm512_permute_epi32_null_permute():
    s = Solver()

    input_vector = zmm_reg("zmm0")
    output_vector = _mm512_permute_ps(input_vector, null_permute_epi32_imm8)
...
def test_mm512_permute_epi32_null_permute_found():
    s = Solver()
...
def test_mm256_permute_epi64_null_permute_works():
    s = Solver()
    input_vector = ymm_reg("ymm0")
    output_vector = _mm256_permute_pd(input_vector, null_permute_pd_imm8)
...
def test_mm256_permute_epi64_null_permute_found():
    s = Solver()
...
def test_mm512_permute_epi64_null_permute_works():
    s = Solver()

    input_vector = zmm_reg("zmm0")
    output_vector = _mm512_permute_pd(input_vector, null_permute_pd_imm8)
...
def test_mm512_permute_epi64_null_permute_found():
    s = Solver()
...
def test_mm256_permutexvar_epi32_null_permute_works():
    s = Solver()
    input = ymm_reg("ymm0")
...
def test_mm256_permutexvar_epi32_null_permute_found():
    s = Solver()
    input = ymm_reg_with_unique_values("ymm0", s, bits=32)
...
def test_mm256_permutexvar_epi32_reverse_permute_found():
    s = Solver()
    input = ymm_reg_with_unique_values("ymm0", s, bits=32)
    indices = ymm_reg("indices")
    output = _mm256_permutexvar_epi32(input, indices)
...
def test_mm512_permutexvar_epi32_null_permute_works():
    s = Solver()
    input_vector = zmm_reg("zmm0")
    indices_vector = zmm_reg_with_32b_values("indices", s, null_permute_vector_epi32_avx512)
    output_vector = _mm512_permutexvar_epi32(input_vector, indices_vector)
...
def test_mm512_permutexvar_epi32_null_permute_found():
    s = Solver()
    input = zmm_reg_with_unique_values("zmm0", s, bits=32)
...
def test_mm512_permutexvar_epi32_reverse_permute_found():
    s = Solver()
    input = zmm_reg_with_unique_values("zmm0", s, bits=32)
    indices = zmm_reg("indices")
    output = _mm512_permutexvar_epi32(input, indices)
...
def test_mm512_permutex2var_epi32_null_permute_works():
    """
    Test that _mm512_permutex2var_epi32 with null permute indices performs
    an identity permutation (selects from source a with identity indices).
    """
    s = Solver()
    
    # Create input vectors with globally unique values  
    a, b = zmm_reg_pair_with_unique_values("input", s, bits=32)
    
    # Create index vector that selects from source a (selector=0) with identity indices
    ...
    # Assert that the output is NOT equal to source a
...
def test_mm512_permutex2var_epi32_null_permute_found():
    """
    Test that Z3 can find the correct index vector for identity permutation from source a.
    """
    s = Solver()
    ...
    # Assert that the output equals source a (seeking identity permutation)
    s.add(a == output)
    result = s.check()
...
def test_mm512_permutex2var_epi32_select_from_b():
    """
    Test that we can select all elements from source b using selector bit = 1.
    """
    s = Solver()
    ...
    # Create index vector that selects from source b (selector=1) with identity indices
    # Each index = (1 << 4) | i for identity indices from source b
    select_b_indices = [(1 << 4) | i for i in range(16)]
    indices = zmm_reg_with_32b_values("indices", s, select_b_indices)
    
    output = _mm512_permutex2var_epi32(a, indices, b)
    
    # Assert that the output is NOT equal to source b
    # If this is unsatisfiable, it means the output MUST be equal to source b
    s.add(b != output)
    result = s.check()
    assert result == unsat, f"Z3 found a counterexample where select from b failed: {s.model() if result == sat else 'No model'}"


def test_mm512_permutex2var_epi32_reverse_permute_from_a():
    """
    Test that we can create a reverse permutation from source a.
    """
    s = Solver()
    ...
    # Create index vector that selects from source a with reverse indices
    # Each index = (0 << 4) | (15 - i) for reverse indices from source a
...
def test_mm512_permutex2var_epi32_mixed_sources():
    """
    Test mixing elements from both sources: even positions from a, odd positions from b.
    """
    s = Solver()
    ...
    # Create index vector: even positions select from a, odd positions select from b
    # Even: (0 << 4) | i, Odd: (1 << 4) | i
    mixed_indices = []
    for i in range(16):
        if i % 2 == 0:
    ...
    # Build expected result: interleaved elements from a and b
    expected_specs = []
    for i in range(16):
        if i % 2 == 0:
...
def test_mm512_permutex2var_epi64_null_permute_works():
    s = Solver()
...
def test_mm512_permutex2var_epi64_null_permute_found():
    s = Solver()
...
def test_mm512_permutex2var_epi64_select_from_b():
    s = Solver()
...
def test_mm512_permutex2var_epi64_reverse_permute_from_a():
    s = Solver()
...
def test_mm512_permutex2var_epi64_mixed_sources():
    s = Solver()
...
def test_mm256_permutexvar_epi64_null_permute_works():
    s = Solver()
    input_vector = ymm_reg("ymm0")
    indices = ymm_reg_with_64b_values("indices", s, null_permute_vector_epi64_avx2)
    output_vector = _mm256_permutexvar_epi64(input_vector, indices)
...
def test_mm256_permutexvar_epi64_null_permute_found():
    s = Solver()
    input = ymm_reg_with_unique_values("ymm0", s, bits=64)
    indices = ymm_reg("indices")
...
def test_mm256_permutexvar_epi64_reverse_permute_found():
    s = Solver()
    input = ymm_reg_with_unique_values("ymm0", s, bits=64)
    indices = ymm_reg("indices")
    output = _mm256_permutexvar_epi64(input, indices)
...
def test_mm512_permutexvar_epi64_null_permute_works():
    s = Solver()
    input = zmm_reg("zmm0")
    indices = zmm_reg_with_64b_values("indices", s, null_permute_vector_epi64_avx512)
    output_vector = _mm512_permutexvar_epi64(input, indices)
...
def test_mm512_permutexvar_epi64_null_permute_found():
    s = Solver()
    input = zmm_reg_with_64b_values("zmm0", s, [i + 1 for i in range(8)])
    permute_indices = zmm_reg("indices")
    output = _mm512_permutexvar_epi64(input, permute_indices)

    s.add(input == output)
    result = s.check()

    assert result == sat, "Z3 failed to find null permute"
    model_indices = s.model().evaluate(permute_indices).as_long()
    expected_long = array_to_long(null_permute_vector_epi64_avx512, bits=64)
    assert model_indices == expected_long, "Z3 found unexpected null permute: got 0x{model_indices:0128x}, expected 0x{expected_long:0128x}"


def test_mm512_permutexvar_epi64_reverse_permute_found():
    s = Solver()
    input = zmm_reg_with_64b_values("zmm0", s, [i + 1 for i in range(8)])
    permute_indices = zmm_reg("indices")
    output = _mm512_permutexvar_epi64(input, permute_indices)
...
def test_mm256_shuffle_ps_null_permute_works():
    s = Solver()
    
    input_vector = ymm_reg("ymm0")
    output_vector = _mm256_shuffle_ps(input_vector, input_vector, null_shuffle_ps_imm8)
...
def test_mm256_shuffle_ps_null_permute_found():
    s = Solver()
    
    input_vector = ymm_reg_with_unique_values("ymm0", s, bits=32)
    imm8 = BitVec("imm8", 8)
    output = _mm256_shuffle_ps(input_vector, input_vector, imm8)
    ...
    model_imm8 = s.model().evaluate(imm8).as_long()
    assert model_imm8 == null_shuffle_ps_imm8, f"Z3 found unexpected null shuffle: got 0x{model_imm8:02x}, expected 0x{null_shuffle_ps_imm8:02x}"


def test_mm256_shuffle_ps_null_permute_2vec_works():
    s = Solver()
    
    op1, op2 = ymm_reg_pair_with_unique_values("op", s, bits=32)
    
    output = _mm256_shuffle_ps(op1, op2, null_shuffle_ps_imm8)
...
def test_mm256_shuffle_ps_null_permute_2vec_found():
    s = Solver()
...
def test_mm512_shuffle_ps_null_permute_works():
    s = Solver()
    
    input_vector = zmm_reg("zmm0")
    output_vector = _mm512_shuffle_ps(input_vector, input_vector, null_shuffle_ps_imm8)
...
def test_mm512_shuffle_ps_null_permute_found():
    s = Solver()
    
    input_vector = zmm_reg_with_unique_values("zmm0", s, bits=32)
    imm8 = BitVec("imm8", 8)
    output = _mm512_shuffle_ps(input_vector, input_vector, imm8)
...
def test_mm512_shuffle_ps_null_permute_2vec_works():
    s = Solver()
    
    op1, op2 = zmm_reg_pair_with_unique_values("op", s, bits=32)
    
    output = _mm512_shuffle_ps(op1, op2, null_shuffle_ps_imm8)
...
def test_mm512_shuffle_ps_null_permute_2vec_found():
    s = Solver()
...
def test_mm256_permute2x128_si256_null_permute_works():
    s = Solver()
...
def test_mm256_permute2x128_si256_null_permute_found():
    s = Solver()
...
def test_mm256_permute2x128_si256_null_permute_2vec_works():
    s = Solver()
...
def test_mm256_permute2x128_si256_null_permute_2vec_found():
    s = Solver()
...
def test_mm256_permute2x128_si256_swap_lanes(): 
    s = Solver()
...
def test_mm256_permute2x128_si256_cross_vector():
    s = Solver()
...
def test_mm256_permute2x128_si256_zero_lanes():
    s = Solver()
...
def test_mm256_permute2x128_si256_zero_both_lanes():
    s = Solver()
...
def test_mm512_shuffle_i32x4_null_permute_works():
    s = Solver()
...
def test_mm512_shuffle_i32x4_null_permute_found():
    s = Solver()
...
def test_mm512_shuffle_i32x4_null_permute_2vec_works():
    s = Solver()
...
def test_mm512_shuffle_i32x4_null_permute_2vec_found():
    s = Solver()
...
def test_mm512_shuffle_i32x4_cross_lanes():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_mask_all_zeros():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_mask_all_ones():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_alternating_mask():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_reverse_with_partial_mask():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_mixed_sources_with_mask():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_single_bit_mask():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_find_identity_mask():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_find_full_permute_mask():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_find_partial_mask():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_find_indices_with_mask():
    s = Solver()
...
def test_mm512_mask_permutex2var_ps_find_reverse_partial():
    s = Solver()
