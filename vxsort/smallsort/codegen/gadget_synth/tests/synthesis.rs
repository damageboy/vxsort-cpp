use gadget_synth::{
    Arch, DType, GadgetGraph, GadgetSynthesizer, InputRef, IntrinsicNode, Operand, OperandBinding,
    OperandKey, Symbolic, SynthesisOptions, VectorState,
};
...
fn permute_pd_node(input_name: &'static str, imm_name: &str) -> IntrinsicNode {
    IntrinsicNode::new("_mm256_permute_pd").with_operands([
        OperandBinding::new(OperandKey::A, input(input_name)),
        OperandBinding::new(OperandKey::Imm8, imm8_symbol(imm_name)),
    ])
}

fn permute2x128_node(input_name: &'static str, imm_name: &str) -> IntrinsicNode {
    IntrinsicNode::new("_mm256_permute2x128_si256").with_operands([
        OperandBinding::new(OperandKey::A, input(input_name)),
        OperandBinding::new(OperandKey::B, input(input_name)),
        OperandBinding::new(OperandKey::Imm8, imm8_symbol(imm_name)),
    ])
}
