...
use gadget_synth::{
    ...
    IntrinsicNode, MuxNode, MuxPruning, OperandBinding, OperandKey, Symbolic, SynthesisOptions,
    VectorState,
};
...
fn permute_pd_node(input: GadgetNode, imm_name: &str) -> IntrinsicNode {
    IntrinsicNode::new("_mm256_permute_pd").with_operands([
        OperandBinding::new(OperandKey::A, input),
        OperandBinding::new(OperandKey::Imm8, imm8_node(imm_name)),
    ])
}
...
fn mux_graph_normalization_includes_select_role_sources_and_pruning() {
    let graph = GadgetGraph::new(
        Some(IntrinsicNode::new("_mm256_permute_pd").with_operands([
            OperandBinding::new(
                OperandKey::A,
            ...
            OperandBinding::new(OperandKey::Imm8, imm8_node("imm8_raw_name")),
        ])),
        None,
    );
...
fn mux_evaluation_concretizes_selected_source() {
    ...
    let graph = GadgetGraph::new(
        Some(IntrinsicNode::new("_mm256_permute_pd").with_operands([
            ...
            OperandBinding::new(OperandKey::Imm8, imm8_node("imm8_mux_eval")),
        ])),
        None,
    );
