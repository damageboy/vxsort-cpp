...
fn intrinsic_uses_only_intrinsics(node: &IntrinsicNode, allowed: &BTreeSet<String>) -> bool {
    allowed.contains(node.name)
        && node
            .operands
            .iter()
            .all(|binding| operand_uses_only_intrinsics(&binding.node, allowed))
}
...
fn normalize_intrinsic_node(node: &IntrinsicNode, context: &mut NormalizationContext) -> Value {
    ...
    sorted_operands.sort_by_key(|binding| binding.key.as_str());
    for binding in sorted_operands {
        operands.insert(
            binding.key.as_str().to_owned(),
            normalize_operand(&binding.node, context, Some(binding.key.as_str())),
        );
    }
