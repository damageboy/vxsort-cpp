...
def test_wave_progress_app_uses_up_down_stage_bindings():
    ...
    assert any(
        binding.key == "tab"
        and binding.action == "toggle_focus"
        and binding.priority is True
        for binding in WaveProgressApp.BINDINGS
        if hasattr(binding, "key")
    )
    assert ("up", "previous_stage", "Prev Stage") in WaveProgressApp.BINDINGS
    assert ("down", "next_stage", "Next Stage") in WaveProgressApp.BINDINGS
    assert ("left", "previous_stage", "Prev Stage") not in WaveProgressApp.BINDINGS
    assert ("right", "next_stage", "Next Stage") not in WaveProgressApp.BINDINGS
...
def test_wave_progress_app_status_text_mentions_up_down_navigation():
    ...
    assert "Up/Down navigate focused pane" in status_text
