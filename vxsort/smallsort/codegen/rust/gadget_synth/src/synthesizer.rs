...
use crate::types::{
    ...
    MuxNode, MuxPruning, PermutationGadget, Symbolic, SynthesisError, SynthesisOptions,
    VectorState,
};
...
impl GadgetSynthesizer {
    ...
    fn constrain_input_register(&self, solver: &Solver, reg: &BV, values: &[u64]) {
        let lane_bits = self.lane_bits();
        for (lane_idx, value) in values.iter().enumerate() {
            solver.assert(
                extract_element(reg, lane_bits, lane_idx).eq(BV::from_u64(*value, lane_bits)),
            );
        }
    }
        ...
        for lane_idx in 0..lanes {
            ...
            if allow_any_lane_order {
                let pair_options: Vec<Bool> = target_pairs
                    .iter()
                    .map(|(elem_a, elem_b)| {
                        let val_a = BV::from_u64(*elem_a, lane_bits);
                        let val_b = BV::from_u64(*elem_b, lane_bits);
                ...
                solver.assert(top_lane.eq(BV::from_u64(elem_a, lane_bits)));
                solver.assert(bottom_lane.eq(BV::from_u64(elem_b, lane_bits)));
            }
        }
    ...
    fn concretize_root(
        ...
        for (inst_idx, node) in order.iter().enumerate() {
            ...
            for (key, operand) in &node.operands {
                args.insert(
                    *key,
                    self.concretize_operand(operand, inst_idx, model, symbolic_vars, &node_to_idx)?,
                );
            }
...
fn apply_mux_pruning(
    ...
    for (_, operand) in &node.operands {
        if let GadgetNode::Mux(mux) = operand
...
fn wire_hardwired(inst0: &IntrinsicNode, inst1: &IntrinsicNode) -> IntrinsicNode {
    IntrinsicNode {
        ...
        operands: inst1
            ...
            .iter()
            .map(|(key, operand)| {
                let wired_operand = if matches!(operand, GadgetNode::Input(_)) {
...
fn wire_with_muxes(
    ...
    IntrinsicNode {
        ...
        operands: inst1
            ...
            .map(|(key, operand)| {
                let wired_operand =
                    if matches!(operand, GadgetNode::Input(_) | GadgetNode::Intrinsic(_)) {
                        GadgetNode::Mux(MuxNode {
                            select: Symbolic {
                                name: format!("sel_{input_name}_{key}_{}", inst1.name),
                                bit_width: 2,
                            },
...
fn has_direct_symbolic_operand(node: &IntrinsicNode) -> bool {
    node.operands
        .iter()
        .any(|(_, operand)| matches!(operand, GadgetNode::Symbolic(_)))
}

fn register_operand_keys(node: &IntrinsicNode) -> Vec<&'static str> {
    node.operands
        .iter()
        .filter_map(|(key, operand)| matches!(operand, GadgetNode::Input(_)).then_some(*key))
        .collect()
}
...
fn build_shared_prefix_tail(
    ...
    IntrinsicNode {
        ...
        operands: dual
            ...
            .iter()
            .map(|(key, operand)| {
                let wired_operand = if *key == first_reg_key {
                    GadgetNode::Intrinsic(Box::new(reg_a.clone()))
                } else if *key == second_reg_key {
                    GadgetNode::Intrinsic(Box::new(reg_b.clone()))
                } else if let GadgetNode::Symbolic(symbolic) = operand {
                    GadgetNode::Symbolic(Symbolic {
...
fn swap_register_operands(node: &IntrinsicNode) -> IntrinsicNode {
    ...
    let input_indices: Vec<usize> = swapped
        ...
        .iter()
        .enumerate()
        .filter_map(|(idx, (_, operand))| matches!(operand, GadgetNode::Input(_)).then_some(idx))
        .collect();
    assert_eq!(input_indices.len(), 2);
    let first_idx = input_indices[0];
    let second_idx = input_indices[1];
    let first_operand = swapped.operands[first_idx].1.clone();
    swapped.operands[first_idx].1 = swapped.operands[second_idx].1.clone();
    swapped.operands[second_idx].1 = first_operand;
    swapped
}
