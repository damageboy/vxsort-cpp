...
impl UiPackScorer {
    pub fn from_data_dir(data_dir: impl AsRef<Path>, arch: &str) -> Result<Self, String> {
        ...
        let canonical_arch = manifest_data
            ...
            .ok_or_else(|| {
                format!(
                    "uiCA data for {arch} is not present in {}. Run `vxsort-codegen fetch-uica-data --target-cpu {arch}` first.",
                    data_dir.as_ref().display()
                )
            })?;
        let runtime = load_manifest_runtime(&manifest, &canonical_arch).map_err(|error| {
            format!(
                "failed to load uiCA data for {arch} from {}: {error}. Run `vxsort-codegen fetch-uica-data --target-cpu {arch}` first.",
                data_dir.as_ref().display()
            )
        })?;
