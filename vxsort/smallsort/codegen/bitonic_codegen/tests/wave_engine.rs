...
use bitonic_codegen::runtime::NullRuntimeSession;
use bitonic_codegen::scoring::{AssignedPath, GadgetCost, PathCost, Scorer};
use bitonic_codegen::transition_table::{TransitionKey, TransitionTable};
...
fn job_state(engine: &WaveEngine, job: &SynthesisJob) -> gadget_synth::VectorState {
    engine.transition_table().state_as_vector_state(job.input())
}

fn chain_state(index: u8) -> gadget_synth::VectorState {
    let base = index * 8;
    state(
...
fn run_stage_sync(
    ...
    engine.run_stage(stage, None, attempt_budget, output_budget, &mut session)
}
...
fn make_jobs_for_stage_zero_uses_initial_state_and_candidate_indices() {
    ...
    assert_eq!(job_state(&engine, &jobs[0]), *engine.initial_state());
...
fn make_jobs_for_downstream_stage_uses_previous_outputs_and_skips_attempted_pairs() {
    ...
    assert!(
        !jobs
            .iter()
            .any(|job| job_state(&engine, job) == output_a && job.candidate_index() == 0)
    );
    assert!(
        jobs.iter()
            .any(|job| job_state(&engine, job) == output_b && job.candidate_index() == 0)
    );
    assert!(jobs.iter().all(|job| job.stage() == 1));
}
...
fn make_jobs_uses_candidate_windows_across_all_inputs() {
    ...
    assert_ne!(job_state(&engine, &jobs[0]), job_state(&engine, &jobs[1]));
    assert_eq!(job_state(&engine, &jobs[0]), job_state(&engine, &jobs[2]));
    assert_eq!(job_state(&engine, &jobs[1]), job_state(&engine, &jobs[3]));
}
...
fn make_jobs_prioritizes_uncovered_inputs_in_each_candidate_window() {
    ...
    assert_eq!(jobs[0].candidate_index(), 0);
    assert_eq!(job_state(&engine, &jobs[0]), output_b);
    assert_eq!(jobs[1].candidate_index(), 0);
    assert_eq!(job_state(&engine, &jobs[1]), output_a);
}
...
fn run_wave_sync_propagates_new_outputs_to_downstream_stage() {
    ...
    let result = engine
        .run_wave(engine.shallow_candidates().len(), 1, &mut session)
        .expect("wave run should not error");
