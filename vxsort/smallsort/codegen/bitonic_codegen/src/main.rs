...
fn main() {
    ...
    match args.command {
        CliCommand::Solve(solve_args) => {
            let config = exit_on_error(bitonic_codegen::parse_solve_run_config(solve_args));
        ...
        CliCommand::SynthesisWorker(_) => {
            exit_on_error(bitonic_codegen::wave_engine::run_synthesis_worker_stdio());
        }
        ...
        CliCommand::Estimate(estimate_args) => {
            let report = exit_on_error(estimate_solution_json(
                &estimate_args.input,
                bitonic_codegen::EstimateOptions {
...
fn score_solution_json(args: &bitonic_codegen::ScoreJsonArgs) -> Result<(), String> {
...
fn print_dry_run_summary(config: &RunConfig, summary: &DryRunSummary) {
    println!("bitonic-codegen dry run");
...
fn print_run_summary(config: &RunConfig, summary: &RunSummary) {
    println!("bitonic-codegen sync run");
    println!("arch: {}", config.arch.cli_name());
    println!("dtype: {}", config.dtype.cli_name());
