...
pub fn run_solver_with_session(
    ...
    let trace = RuntimeTrace::from_path(config.runtime_trace_path.as_deref())?;
    ...
    for target_cpu in &config.target_cpus {
        ...
        engine.set_runtime_trace(trace.clone());
        ...
        live_full_scorer.set_runtime_trace(trace.clone());
        let result = {
        ...
        let final_paths = match config.prune_score {
            ...
            PruneScoreArg::Uica => live_full_scorer.finish(session, "target_finished")?,
        };
...
impl LiveFullScorer {
    ...
    fn set_runtime_trace(&mut self, runtime_trace: RuntimeTrace) {
    ...
    fn enqueue_retained_paths(
        ...
        for rough_path in rough_paths {
            ...
            crate::trace!(self.trace, "live_full_scoring_job_queued", {
                        "reason": reason,
    ...
    fn handle_completed_result(&mut self, result: FinalScoringResult, reason: &str) {
        ...
        crate::trace!(self.trace, "live_full_scoring_job_completed", {
                    "reason": reason,
                    "target_cpu": self.target_cpu,
    ...
    fn poll(
        ...
        while let Ok(result) = self.receiver.try_recv() {
            let result = result?;
            self.handle_completed_result(result, reason);
        }
        ...
        crate::trace!(self.trace, "live_full_scoring_snapshot", {
                    "reason": reason,
                    "target_cpu": self.target_cpu,
                    "submitted": self.submitted_count,
...
impl WaveProgressObserver for LiveFullScorer {
    fn on_wave_progress(
        ...
        self.enqueue_retained_paths(engine.transition_table(), candidate_paths, reason)?;
        self.poll(engine.scored_paths(), session, reason)
    }
}
...
mod tests {
  ...
  "architectures": {{
    ...
    fn live_full_scoring_deduplicates_retained_rough_paths_and_reports_progress() {
        ...
        scorer
            .enqueue_retained_paths(&table, std::slice::from_ref(&rough_path), "test")
        ...
        for _ in 0..50 {
            scorer
                .poll(std::slice::from_ref(&rough_path), &mut session, "test")
