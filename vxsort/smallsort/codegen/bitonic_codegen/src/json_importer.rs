...
use crate::transition_table::{CompletePath, GadgetIndex, LaneLabel, StateTuple, TransitionTable};
...
fn parse_lane_label_array(value: &Value, label: &str) -> Result<Vec<LaneLabel>, String> {
    parse_u64_array(value, label)?
        .into_iter()
        .map(|entry| {
            LaneLabel::try_from(entry)
                .map_err(|_| format!("{label} entry {entry} does not fit in u8"))
        })
        .collect()
}
