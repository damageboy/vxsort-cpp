...
use crate::transition_table::{CompletePath, LaneLabel, StateTuple, TransitionTable};
...
impl PathVerifier {
    ...
    fn build_register(&self, elems: &[BV], labels: &[LaneLabel]) -> Result<BV, String> {
        ...
        for &label in labels {
            ...
            if label == 0 || label_index > elems.len() {
                return Err(format!(
                    "element label {label} out of range 1..={}",
                    elems.len()
                ));
            }
            lane_elems.push(elems[label_index - 1].clone());
        }
        lane_elems.reverse();
        ...
        (1..=n)
            .map(|label| {
                let label = LaneLabel::try_from(label)
                    .map_err(|_| format!("element label {label} does not fit in u8"))?;
